...
class RouterController
{
	...
	public static function handleStatic($staticDir, $path) {
		...
		if(isset(Router::$mimeTypes[$ext]))
			header("content-type: " . Router::$mimeTypes[$ext]);

		$fullPath = realpath($staticDir . "/" . ltrim($path, "/"));

		if(
			!preg_match("/^".preg_quote(realpath($staticDir), "/")."/", $fullPath)
			or !file_exists($fullPath)
		) return self::handle404();

		if($f = fopen($fullPath, "r")) {

			if(($fs = filesize($fullPath)) !== false)
...
class Router
{
	...
	// NOTE:  Source for this mimetype list is
	// https://roytuts.com/get-mime-type-from-file-extension-using-php/
